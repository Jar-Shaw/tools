...
public class DateUtils {
    ...
    /**
     * 构建符合数据库标准的1900-9999年日期时间正则表达式
     *
     * @return Pattern 符合数据库标准的1900-9999年日期时间正则表达式
     */
    private static Pattern getDateTimeRegex() {

        String month_31 = "(0?[13578]|1[02])";
        String month_30 = "(0?[469]|11)";
        String days_31 = "(0?[1-9]|[12][0-9]|3[01])";
        String days_30 = "(0?[1-9]|[12][0-9]|30)";
        String days_28 = "(0?[1-9]|[1][0-9]|2[0-8])";
        String divide_by_4_zero = "(0[48]|[2468][048]|[13579][26])";
        String divide_by_4 = "([2468][048]|[3579][26])";
        String hour = "([01]?[0-9]|2[0-3])";
        String min_or_sec = "[0-5]?[0-9]";
        String betwenn_19_99 = "(19|[2-9][0-9])";
        String common_days = betwenn_19_99 + "[0-9]{2}-(" + month_31 + "-" + days_31 + "|" + month_30 + "-" + days_30 + "|" + "(02|2)-" + days_28 + ")";
        String feb_29 = "(" + betwenn_19_99 + divide_by_4_zero + "|" + divide_by_4 + "00)-02-29";
        String time = hour + ":" + min_or_sec + "(:" + min_or_sec + ")?";

        String dateRegex = "^(" + common_days + "|" + feb_29 + ")( +" + time + ")?$";
        return Pattern.compile(dateRegex);
    }
    ...
    public static String getDateFormat(String dateStr){
        ...
        dateRegFormat.put(
            ...
            "yyyy-MM-dd-HH-mm");//2014年3月12日 13时5分，2014-03-12 12:05，2014/3/12 12:5
        dateRegFormat.put(
            "^\\d{4}\\D+\\d{1,2}\\D+\\d{1,2}\\D+\\d{1,2}\\D*$",
            "yyyy-MM-dd-HH");//2014年3月12日 13时，2014-03-12 12:，2014/3/12 12:
        dateRegFormat.put(
            "^\\d{4}\\D+\\d{1,2}\\D+\\d{1,2}\\D*$",
            "yyyy-MM-dd");//2014年3月12日，2014-03-12，2014/3/12
        dateRegFormat.put(
            "^\\d{4}\\D+\\d{1,2}\\D*$",
            "yyyy-MM");//2014年3月，2014-03-，2014/3/
        dateRegFormat.put(
            "^\\d{4}\\D*$",
            "yyyy");//2014年
    ...
    /**
     *  将String类型表述的时间转化为标准的Date时间格式
     *  举例：将2015年3月2日转化为标准的date格式
     * @param dateStr 时间字符串
     * @return Date
     * @throws ParseException ParseException
     */
    public static Date getDateFromString(String dateStr) {
        String getDateFormat = getDateFormat(dateStr);
        DateFormat formatter_bff = new SimpleDateFormat(getDateFormat);
        String dateReplace = dateStr.replaceAll("\\D+", "-");
        return ExceptionHelper.runtime(() -> formatter_bff.parse(dateReplace));
    }

    /**
     * 将字符串日期格式化为指定的格式字符串
     * @param dateString 字符串日期
     * @param formatter 要得到的日期格式
     * @return 格式化后的日期字符串
     */
     ...
     * @return 格式化后的日期字符串
     */
    public static String dateAdd(Date date, String formatter)
    {
        date.getDate();
    ...
    /**
     * 得到某一年的清明节是几号，清明是4月的4到6号
     * @param year 年份
     * @return 4或5或6
     */
    public static int getTombDay(int year) {
        return (int) (((year - 2000) * 0.2422 + 4.81) - ((year - 2000) / 4));
    }
